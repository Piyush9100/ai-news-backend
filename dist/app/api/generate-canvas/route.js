...
async function POST(req) {
    try {
        ...
        if (!Array.isArray(headlines) || !category) {
            return server_1.NextResponse.json({ error: "Headlines array & category required" }, { status: 400 });
        }
        ...
        `;
            ...
            await supabase_server_1.supabaseServer.storage
                .from("news-images")
                .upload(filePath, buffer, {
                contentType: "image/png",
                upsert: false,
            });
            const { data } = supabase_server_1.supabaseServer.storage
                .from("news-images")
        ...
        console.error(err);
        return server_1.NextResponse.json({ error: "Canvas generation failed" }, { status: 500 });
    }
}
